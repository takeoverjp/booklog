...
# ルール１７　大きな問題ほど解決しやすいこともある

- 基本的に過度な一般化は無駄に複雑になる
- ただし、まれに一般化した方が解きやすいこともある
- 特定の問題を考えて解決が難しい場合、観点を大きく変えて一般的な問題ととらえ直してみる

# ルール１８　コードに自らの物語を語らせろ

- アサートで記述できる内容はコメントではなくアサートにすることで、メンテ漏れを避けることができる
- 良いコメントとは、コードについて **明白じゃない** 何かを読者に伝えるコメントである

# ルール１９　作り直しは並列で行うこと

- 望ましい変更それぞれについて、その変更を簡単にできるようにして、その上で簡単な変更を行う
- 大規模なAPI変更の場合、新旧APIをフラグで簡単に切り替えられるようにしておき、問題がないかの検証・問題があった時の切り戻しを簡単にできるようにしておいてから、実際に以降を進める
- 元のバージョン、書き直しバージョン、アダプターを管理するコスト、オーバーヘッド、コード量の増加など、デメリットもあるので、それを超えるメリットがある大規模変更の場合に採用する

# ルール２０　計算をやっておけ

- 自動化するか、手動のままでいくかは、かかる時間と浮く時間を計算で比較するべき
  - とんとんなときは、自動化のコストを低く見積もっていることが多いので、手動のままのほうがよい
- ハードリミットは物事を単純化するので、設計プロセスの最初から尊重すべき
  - 組込におけるCPUコア、RAMサイズ、ストレージサイズ、ネットワーク帯域など

# ルール２１　とにかく釘を打たなきゃいけないこともある

- 退屈な作業をスキップしても、誰も代わりにやってくれない
- 長期的な便益より短期的なコストを優先して、後悔することが多い。つべこべ言わずに立ち向かえ！
- 規律正しいチームでも、退屈な作業に飛び込むのは難しい
  - Step1. やりたくないからタスクを避けていることを認識する
  - Step2. タスクに立ち向かった場合の長期的な便益を評価する
  - Step3. 釘を打つ！
