...
- Option<T>とResult<T, E>は非常に一般的なenum型である
  - 存在しない場合がある値を示す場合はOptionを使う
  - 失敗する可能性がある操作の結果はResultを使う
- match式で型安全性を確保する
...
- rust-toolchain.tomlを用いて、CIビルドで用いるバージョンを固定する
- no std互換かどうかをチェックする
- 依存グラフを管理する
...
- coreの主な制約は、「ヒープ上にメモリ確保しない」こと
- no_std環境では、OSによる安全な乱数生成を過程できないので、ハッシュが使えない
- no_std環境では、マルチスレッドの同期機構が使えない
- メモリ確保の失敗の可能性を考慮する必要がある

### 項目 34：FFI 境界を通過するものを制御しよう

- unsafeなFFIライブラリへのアクセスはsafeなRustコードでカプセル化する
- panic!がFFI境界をまたがないようにする
