...
- 論理ゲートには下記のような特徴がある。これらの特徴（制約）を、さまざまなレベルのアイディアや工夫によって乗り越えることで、現代のCPUの性能向上が進んでいる
  - 大きな論理ゲートへ信号を伝えるのは遅い（入力容量が大きいため）
  - 多くの論理ゲートへ信号を伝えるのは遅い（配線容量が大きくなるため）
  - 遠くの論理ゲートへ信号を伝えるのは遅い（配線の容量と抵抗が増加するため）

- 論理ゲートより下位の物理レベルには、下記のような制約がある
  - CMOSトランジスタで構成される論理ゲートにおいて、NORゲートがNANDゲートよりも遅い
  - 半導体の製造技術を越えて細かい半導体は生産できない。すでにゲート長は5nmといったオーダーになっており、原子100個以下のサイズ感になっている
