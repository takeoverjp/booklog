...
- まずロック、スレッドプール、キューを調整する
- I/O, CPU, RAMについては、何が支配的かで戦略が変わる
  - CPU律速:リクエストごとのスレッドアーキテクチャ
  - RAM律速:リクエストごとのスレッドアーキテクチャ
  - I/O律速:他のリソースを活用してI/Oを減らす
  - バランス:スレッドを増やす(コア数の20~50倍)か、イベント駆動型

## 12. 安定したシステムの構築

- 安定性とは、第一に不可逆の不整合を起こさないこと、そしてその次にダウンタイムが少ないことである
- アドミッション制御として、タイムアウトやキューの上限を設けることで、負荷の問題を予測可能にする
- 遅いクライアントが負荷の原因になることがある
- 障害を意図的に起こし、リカバリパスの検証を行うことが重要
- 人的ミスは、GitOps(IaC)やブルーグリーンデプロイメント、カナリアデプロイメントで対処する

## 13. システムの構築と進化

- ユニットテストは容易に書けるべきだが、どのフレームワーク・ツールを使うべきかの選択を開発者に丸投げしてはならない。ユニットテストをうまく書けないときはアーキテクトに相談するようにお願いする
- ほとんどのプロジェクトの失敗は、基本を疎かにしたことによる。基本を適切かつ継続的に維持する。
- ビジネスは驚きを嫌う。早めにコミュニケーションをとり、理由を説明する。
