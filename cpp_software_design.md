...
- Bridgeパターンはメンバ変数などの実装詳細を依存先から隠蔽するためのパターン
  - Bridgeという名前から繋げる、距離を近づけるイメージを受けるかもしれないが、むしろ適切に距離をとることが目的
- pimlイディオムは、ローカルで多態性を持たない形のBridgeパターン
- `pimpl_`メンバ変数を`std::unique_ptr`にしても、デストラクタを自分で実装する必要がある（「手動でのメモリ解放が必要」は誤訳）
  - なぜなら、デストラクタを省略すると、ヘッダ内にデストラクタが定義され、Implクラスのデストラクタ定義がヘッダに必要になってしまうため
  - ヘッダではデストラクタの宣言のみし、ソースでデストラクタの定義(`=default`で十分)をする
...
- Bridgeパターンのデメリット
  ...
  - メンバ関数がインライン展開されなくなることによる性能オーバーヘッド
    - 仮にメンバ変数にアクセスするだけの非常にシンプルなメソッドであっても、インライン展開できなくなる
...
- CPUのキャッシュラインを有効に使えるように、メンバ変数の配置を工夫する
- 性能に関しては、勘に頼らずベンチマーク測定を常に確認する
- どこまでのメンバ変数を隠蔽のためにimplに移動し、どこまでのメンバ変数を性能のために埋め込むかは、状況による。計測結果に基づき、最適解を見つけなければならない

## ガイドライン３０：コピー演算を抽象化するにはPrototypeパターン

- Prototypeパターンは、基底クラスへのポインタしかなく、具象クラスがわからない状態でコピーするためのパターンである。
- Prototypeパターンは、生成すべきオブジェクトの種類を原型となるインスタンスを使って明確にし、それをコピーすることで新たなオブジェクトの生成を行う
- 参照セマンティクスの解しかなく、値セマンティクスの解はない
- `clone`という関数名はまず間違いなくPrototypeパターンを表す
...
- 実行時多態（継承）とコンパイル時多態（テンプレート）を適材適所で組み合わせて実現している
...
- Adapterパターンとの違い
  - Adapterパターンは、インターフェースの変換が目的。既存のインターフェースに変換しているなら、こちら。
...
- 目的
  - 次の性質を持つ拡張可能な型セットを対象に、既存コードに干渉せず値ベースに抽象化する
    - コピー可能、ムーブ可能、推論可能な値型
